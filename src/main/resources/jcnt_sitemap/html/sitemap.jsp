...
<a href='<c:url value="${url.base}${currentNode.properties.base.node.path}.html"/>'>${currentNode.properties.base.node.properties["jcr:title"].string}
    level ${level}</a>
<c:forEach items="${jcr:getChildrenOfType(currentNode.properties.base.node,'jcmix:sitemap')}" var="child" varStatus="childStatus">
    <c:if test="${childStatus.first}">
        <ul <c:if test="${level eq 1}"><c:set var="nbSubItems" value="${jcr:getChildrenOfType(currentNode.properties.base.node,'jcmix:sitemap')}"/> id="primaryNav" class="col${fn:length(nbSubItems)}"</c:if>>
    </c:if>
